...
export function startHelperLines(target) {
    function showHelperLinesAt(pdfField, coords) {
        const calculate = {
        ...
        const rect = pdfField.getBoundingClientRect();
        const positions = {
            top: (coords && coords.y) || rect.top,
            left: (coords && coords.x) || rect.left,
            height: pdfField.clientHeight,
            width: pdfField.clientWidth,
        };
 ...
 */
export function startResize(pdfField, onResize) {
    const page = pdfField.el.parentElement;
    const mouse = {};
    const resizeHandleWidth = pdfField.el.querySelector(".resize_width");
    const resizeHandleHeight = pdfField.el.querySelector(".resize_height");
    const resizeHandleBoth = pdfField.el.querySelector(".resize_both");

    const computeDimensions = (e) => {
        const { direction, x, y } = mouse;
        const computedStyle = getComputedStyle(pdfField.el);
        const pdfFieldAbsoluteWidth = parseInt(computedStyle.width);
        const pdfFieldAbsoluteHeight = parseInt(computedStyle.height);
        ...
        const factor = {
            x: (dX + pdfFieldAbsoluteWidth) / pdfFieldAbsoluteWidth,
            y: (dY + pdfFieldAbsoluteHeight) / pdfFieldAbsoluteHeight,
        };

        if (dX < 0 && Math.abs(dX) >= pdfFieldAbsoluteWidth) {
            factor.x = 1;
        }

        if (dY < 0 && Math.abs(dY) >= pdfFieldAbsoluteHeight) {
            factor.y = 1;
        }

        var options = pdfField.data.props.fieldInfo.position;
        const width =
            direction === "width" || direction === "both"
                ? Math.round(
                      normalizeDimension(factor.x * options.width, options.posX) * 1000
                  ) / 1000
                : options.width;
        ...
        const height =
            direction === "height" || direction === "both"
                ? Math.round(
                      normalizeDimension(factor.y * options.height, options.posY) * 1000
                  ) / 1000
                : options.height;
    ...
    const handleMouseMove = (e) => {
        if (pdfField.el.classList.contains("o_resizing")) {
            e.preventDefault();
            onResize(pdfField, computeDimensions(e), false);
        }
    };
    ...
    const handleMouseDown = (e, direction) => {
        e.preventDefault();
        pdfField.el.classList.add("o_resizing");
    ...
    page.addEventListener("mouseup", (e) => {
        if (pdfField.el.classList.contains("o_resizing")) {
            pdfField.el.classList.remove("o_resizing");
            page.removeEventListener("mousemove", debouncedOnMouseMove);
            onResize(pdfField, computeDimensions(e), true);
        }
    });
}
