...
class StockInventory(models.Model):
    ...
    adjustment_reason_id = fields.Many2one('stock.adjustment.reason',string='Adjustment Reason',help="Select the adjustment reason for this inventory adjustment")
    ...
    product_ids = fields.Many2many(
        ...
        check_company=True,
        readonly=True,
        domain="[('type', '=', 'cosnu'), ('company_id', 'in', [False, company_id])]",
    ...
    def create(self, vals):
        if not vals.get('name'):
            vals['name'] = self.env['ir.sequence'].next_by_code('stock.inventory.sequence')
    ...
    #         self.analytic_account_id = self.location_ids[0].analytic_account_id
    ...
    def post_inventory(self):
        ...
        for inventory in acc_inventories:
            self.with_context(force_period_date=inventory.accounting_date,ac_include_analytic_account=True).mapped(
                "move_ids"
            ).filtered(lambda move: move.state != "done")._action_done()
        other_inventories = self - acc_inventories
        if other_inventories:
            self.mapped("move_ids").with_context(ac_include_analytic_account=True).filtered(
    ...
    def action_check(self):
        ...
        for inventory in self.filtered(lambda x: x.state not in ("done", "cancel")):
            ...
            if inventory.move_ids and inventory.mapped("move_ids"):
    ...
    def action_open_inventory_lines(self):
        ...
        action["view_id"] = self.env.ref(
            "ak_inventory_adjustments.stock_inventory_line_list"
        ).id
        if self.product_ids:
...
class AdjustmentReason(models.Model):
    ...
    name = fields.Char(string='Adjustment Reason', required=True)
