...
function computeWidths(table, state, allowedWidth, startingWidths) {
    ...
    for (let columnIndex = 0; columnIndex < columns.length; columnIndex++) {
        ...
        if (
            state.self.props.list.resModel &&
            fieldName &&
            browser.localStorage
        ) {
            const storedWidth = browser.localStorage.getItem(
                `odoo.columnWidth.${state.self.props.list.resModel}.${fieldName}`
            );
            if (storedWidth) {
                _columnWidths[thIndex] = parseInt(storedWidth, 10);
            }
        }
    }
...
export function useMagicColumnWidths(tableRef, getState) {
    ...
    function onStartResize(ev) {
        ...
        const saveWidth = (saveWidthEv) => {
            ...
            ev.preventDefault();
            ev.stopPropagation();
            const fieldName = th ? th.dataset.name : undefined;
