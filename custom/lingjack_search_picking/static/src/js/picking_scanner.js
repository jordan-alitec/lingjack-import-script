...
const createModalHTML = () => {
    return `
        <div id="picking-scanner-modal" style="
            position: fixed;
            top: 0;
            left: 0;
            width: 100%;
        ...
        ">
            <div style="
                ...
                text-align: center;
            ">
                <h3 style="margin: 0 0 20px 0; color: #333;">Scan Picking Barcode</h3>
                ...
                <div id="qr-reader-picking" style="
                    width: 100%;
                    max-width: 400px;
                ...
                ">
                    ...
                    Point camera at picking barcode
                </div>
            </div>
        </div>
    `;
...
const initScanner = () => {
    ...
    const existingModal = document.getElementById('picking-scanner-modal');
    if (existingModal) {
...
const startScanner = async () => {
    try {
        ...
        const qrReaderElement = document.getElementById('qr-reader-picking');
        if (!qrReaderElement) {
        ...
        scanner = new Html5Qrcode('qr-reader-picking');
...
const handleScanResult = async (decodedText) => {
    try {
        ...
        // Search for picking directly
        const pickings = await rpc('/web/dataset/call_kw', {
            model: 'stock.picking',
            method: 'search_read',
            args: [[['name', 'ilike', decodedText]]],
            kwargs: {
                fields: ['id', 'name'],
                limit: 1
            }
        ...
        if (pickings && pickings.length > 0) {
            ...
            // Open picking record directly
            window.location.href = `/web#id=${pickings[0].id}&model=stock.picking&view_type=form`;
        } else {
            updateStatus('No picking found: ' + decodedText, '#dc3545');
            setTimeout(() => {
 ...
 */
registry.category("actions").add("picking_barcode_scanner", async (env, action) => {
    try {
        console.log('Picking barcode scanner invoked', action);
        ...
        console.error('Failed to execute picking barcode scanner:', error);
