#!/usr/bin/env python3
"""
Odoo 18 SetscoList.xlsx Import Script (BOM/Setsco_List/)
...
Expects SetscoList.xlsx in this folder (Setsco_List) by default.
Standalone: no dependency on BOM/Setsco/ or import_setsco_serials_to_odoo.
"""
...
# Path setup: config from BOM parent
script_dir = Path(__file__).parent  # Setsco_List
...
#     IMPORT_OFFICE = getattr(config, 'SETSCO_LIST_IMPORT_OFFICE', True)
#     IMPORT_WAREHOUSE = getattr(config, 'SETSCO_LIST_IMPORT_WAREHOUSE', True)
#     IMPORT_PRODUCTION = getattr(config, 'SETSCO_LIST_IMPORT_PRODUCTION', False)
# except ImportError:
...
IMPORT_OFFICE = False
IMPORT_WAREHOUSE = False
IMPORT_PRODUCTION = True
...
# TUV sheet codes for serial_type (same as Setsco combined import)
TUV_SHEET_CODES = ['03071601', '03071602', '03071606']


class SetscoListImporter:
    """Standalone importer for SetscoList.xlsx (Office, Warehouse, Production sheets)."""
    ...
    # --------------- Lookups (setsco category, product) ---------------

    def find_setsco_category_by_description(self, sheet_name: str) -> Optional[int]:
        """Find setsco.category by description (ilike)."""
        if not sheet_name:
            return None
    ...
    def find_product_by_default_code(self, default_code: Any) -> Optional[int]:
        """Find product.product by default_code (Com No)."""
    ...
    def determine_serial_type(self, category_str: str) -> str:
        """Return 'tuv' if category contains TUV code, else 'setsco'."""
        if not category_str:
            return 'setsco'
        category_str = str(category_str).strip()
        for tuv_code in TUV_SHEET_CODES:
            if tuv_code in category_str:
                return 'tuv'
        return 'setsco'
    ...
    # --------------- Location / warehouse / production helpers ---------------
    ...
    def pre_run(
        self,
        excel_file: str,
        import_office: bool = True,
        import_warehouse: bool = True,
        import_production: bool = False,
    ) -> Tuple[bool, List[str]]:
        """
        Validate all rows for enabled sheets: Com No (when required) and location/category exist.
        ...
        sheet_flags = {
            SHEET_OFFICE: import_office,
            SHEET_WAREHOUSE: import_warehouse,
            SHEET_PRODUCTION: import_production,
        }

        # Check warehouse 1 and Stock location once (needed for Warehouse and Production)
        if import_warehouse or import_production:
        ...
        for sheet_name in SHEETS_ORDER:
            if not sheet_flags.get(sheet_name, False):
                continue
    ...
    def import_from_excel(
        ...
        """
        Run pre-run for enabled sheets; if pass (or force), import those sheets.
        ...
        logger.info(f"Sheets: Office={import_office}, Warehouse={import_warehouse}, Production={import_production}")
        logger.info("=" * 80)
        ...
        passed, errors = self.pre_run(
            excel_file,
            import_office=import_office,
            import_warehouse=import_warehouse,
            import_production=import_production,
        )
        for err in errors:
            logger.error(err)
        ...
        for sheet_name in SHEETS_ORDER:
            if sheet_name not in xl.sheet_names:
                continue
            if sheet_name == SHEET_OFFICE and not import_office:
                continue
            if sheet_name == SHEET_WAREHOUSE and not import_warehouse:
                continue
            if sheet_name == SHEET_PRODUCTION and not import_production:
                continue
            df = pd.read_excel(excel_file, sheet_name=sheet_name)
...
def main():
    ...
    parser.add_argument('--file', type=str, default=None,
                        help='Path to SetscoList.xlsx (default: SetscoList.xlsx in this folder)')
    parser.add_argument('--office', action='store_true', default=None,
                        help='Import Office sheet (default: from config)')
    parser.add_argument('--no-office', action='store_true', help='Skip Office sheet')
    parser.add_argument('--warehouse', action='store_true', default=None,
                        help='Import Warehouse sheet (default: from config)')
    parser.add_argument('--no-warehouse', action='store_true', help='Skip Warehouse sheet')
    parser.add_argument('--production', action='store_true',
                        help='Import Production sheet (default: off, run Office+Warehouse first)')
    parser.add_argument('--no-production', action='store_true', help='Explicitly skip Production sheet')
    args = parser.parse_args()

    import_office = IMPORT_OFFICE if args.office is None and not args.no_office else (args.office or False)
    import_warehouse = IMPORT_WAREHOUSE if args.warehouse is None and not args.no_warehouse else (args.warehouse or False)
    import_production = IMPORT_PRODUCTION or args.production
    if args.no_production:
        import_production = False
    ...
    try:
        ...
        if args.pre_run:
            passed, errors = importer.pre_run(
                excel_file,
                import_office=import_office,
                import_warehouse=import_warehouse,
                import_production=import_production,
