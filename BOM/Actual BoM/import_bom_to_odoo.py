...
class OdooBoMImporter:
    ...
    def create_bom(
        ...
        lingjack_product_category: str = None,
    ) -> int:
        """
        Create a BoM record
        ...
        bom_vals = {
            ...
            'empty_cabinet_bom': False,
        }
        
        if product_name:
            bom_vals['display_name'] = product_name

        # Set Lingjack product category if provided
        if lingjack_product_category:
            bom_vals['lingjack_product_category'] = str(lingjack_product_category).strip()
    ...
    def parse_excel(self, excel_path: str) -> List[Dict]:
        ...
        for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            # Column 10 (1-based) = index 9 (0-based) for Lingjack Product Category
            lingjack_product_category = row[9] if len(row) > 9 else None
            ...
            if product_name and str(product_name).strip():
                ...
                current_bom = {
                    ...
                    'components': [],
                    'lingjack_product_category': str(lingjack_product_category).strip()
                    if lingjack_product_category
                    else None,
                }
    ...
    def import_boms(self, excel_path: str, dry_run: bool = False) -> Dict:
        ...
        for bom_idx, bom_data in enumerate(boms, 1):
            try:
                ...
                # Create BoM
                if not dry_run:
                    bom_id = self.create_bom(
                        product_id,
                        bom_data['product_name'],
                        bom_data.get('lingjack_product_category'),
                    )
                else:
                    bom_id = None
...
def main():
    ...
    try:
        ...
        ODOO_DB = "your_database_name"  # Change to your database name
        ODOO_USERNAME = "admin"  # Change to your username
